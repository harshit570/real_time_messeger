...
export const sendMessageService = async (
  ...
  if (replyToId) {
    ...
    if (!replyMessage) throw new NotFoundException("Replied Message Not Found");
  }
  let imageUrl;
  if (image) {
...
async function getAIResponse(chatId: string, userId: string) {
  ...
  const result = await streamText({
    ...
    system:
      "You are Whop AI, a helpful assistant. Respond only with text and attend to the last user message only",
  });
